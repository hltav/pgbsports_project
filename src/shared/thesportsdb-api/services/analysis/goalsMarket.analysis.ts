...
// Opções: "Par", "Ímpar"
export function analyzeParOuImpar(
  eventDetails: string,
  homeScore: number,
  awayScore: number,
): EventMarketAnalysis {
  const totalGols = homeScore + awayScore;
  const ehPar = totalGols % 2 === 0;

  if (eventDetails.toLowerCase().includes('par')) {
    return {
      result: ehPar ? Result.win : Result.lose,
      shouldUpdate: true,
    };
  }

  if (eventDetails.toLowerCase().includes('ímpar')) {
    return {
      result: !ehPar ? Result.win : Result.lose,
      shouldUpdate: true,
    };
...
export function analyzeNumeroParImpar(
  ...
  const isPar = normalized.includes('par') && !normalized.includes('ímpar');
  const isImpar = normalized.includes('ímpar');
