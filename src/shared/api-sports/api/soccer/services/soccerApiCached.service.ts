...
import { SoccerApiService } from './soccerApi.service';
import {
  HistoricalParams,
  LiveParams,
  SportsApiParams,
...
@Injectable()
export class SoccerApiCachedService {
  constructor(
    private readonly sportsApiService: SoccerApiService,
  ...
  private calculateTtl(params?: SportsApiParams, customTtl?: number): number {
    ...
    if (this.isHistoricalData(params)) {
      return CACHE_TTL.ONE_MONTH; // 30 days
    }

    if (this.isLiveData(params)) {
      return CACHE_TTL.TEN_SECONDS; // 10 seconds
    }

    if (this.isTeamsParams(params)) {
      return CACHE_TTL.ONE_DAY; // 24 hours
    }

    if (this.isStandingsParams(params)) {
      return CACHE_TTL.THIRTY_MINUTES; // 30 minutes
    }

    return CACHE_TTL.ONE_HOUR; // Default: 1 hour
  }
