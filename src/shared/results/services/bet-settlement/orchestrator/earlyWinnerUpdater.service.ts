...
export class EarlyWinnerUpdateService {
  ...
  // ─── CHAMADO PELO CRON — apenas enfileira ────────────────────────────────────

  async updateEarlyWinnerBets(): Promise<number> {
    this.logger.debug(
      '⚡ Queueing early-winner bets (market="Resultado Antecipado")',
    );
    ...
    // ─── Separação inicial (igual antes, mas só com IDs) ─────────────────────
    const betsByKey = new Map<string, number[]>();

    for (const bet of bets) {
      const tsdbEventId = bet.externalMatch?.tsdbEventId;
      const status = bet.externalMatch?.status;

      // sem TSDB e não FINISHED → ainda não pode processar
      if (!tsdbEventId && status !== MatchStatus.FINISHED) {
        continue;
      }

      const key = tsdbEventId
        ? `tsdb:${tsdbEventId}`
        : `ext:${bet.externalMatch.id}`;

      if (!betsByKey.has(key)) betsByKey.set(key, []);
      betsByKey.get(key)!.push(bet.id);
    }

    if (betsByKey.size === 0) return 0;

    const { queued } =
      await this.queueService.enqueueEarlyWinnerBatch(betsByKey);

    this.logger.log(`📬 ${queued} early-winner jobs enqueued`);

    return queued;
  }

  // ─── CHAMADO PELO WORKER — processa um grupo por vez ────────────────────────

  async processEarlyWinnerGroupByIds(
    key: string,
    betIds: number[],
  ): Promise<number> {
    const bets = await this.prisma.bets.findMany({
      ...
      include: {
        externalMatch: {
          include: { externalMatchDetails: true },
        },
      },
    });
    ...
    return this.processGroup(key, bets as BetWithExternalMatchDetails[]);
  }

  // ─── LÓGICA DE PROCESSAMENTO (loop original movido para ca — sem mudanca na logica) ──

  private async processGroup(
    key: string,
    eventBets: BetWithExternalMatchDetails[],
  ): Promise<number> {
    const betRef = eventBets[0];
    const externalMatch = betRef.externalMatch;
    const details = externalMatch.externalMatchDetails;
